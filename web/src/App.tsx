...
function statesEqual(a: UpgradeState, b: UpgradeState): boolean {
  for (const tier of [1, 2, 3, 4] as const) {
    const al = a.levels[tier];
    const bl = b.levels[tier];
  ...
  for (let i = 0; i < 4; i += 1) {
    if (a.gemLevels[i] !== b.gemLevels[i]) return false;
  }
  return true;
}
...
function toBudget(s: UiState): Budget {
  return {
    1: Math.max(0, parseNumber(s.budget1)),
    2: Math.max(0, parseNumber(s.budget2)),
    3: Math.max(0, parseNumber(s.budget3)),
    4: Math.max(0, parseNumber(s.budget4)),
  };
}
...
export function App() {
  ...
  // Greedy optimizer button intentionally removed in web parity mode.
  ...
  return (
    <div className="container">
      <div className="header">
        <div>
          <h1 className="title">ObeliskFarm (Web) — Event Budget Optimizer</h1>
          <p className="subtitle">
            Branch <span className="mono">web</span>. Saves upgrades/prestige automatically in your browser (localStorage).
          </p>
        </div>
                            ...
                            <div className="small">
                              {unlocked ? (
                                <>
                                  ...
                                  / <span className="mono">{max}</span> • next cost{" "}
                                  <span className="mono">{formatInt(nextCost)}</span>
                                </>
                              ) : (
                                <>
                                  <span className="pillLocked">LOCKED</span>{" "}
                                  <span className="lockedText">until prestige {PRESTIGE_UNLOCKED[t][idx]}</span>
                                </>
                              )}
                            </div>
                          </div>
            ...
            {result ? (
              <>
                ...
                {[1, 2, 3, 4].map((tier) => {
                  ...
                  const initial = lastInitialRef.current?.levels?.[tier as 1 | 2 | 3 | 4] ?? null;
                  const picked = levels
                    .map((lvl, idx) => ({ lvl, idx }))
                    .filter((x) => (initial ? x.lvl > (initial[x.idx] ?? 0) : x.lvl > 0))
                    .map((x) => ({ ...x, add: initial ? x.lvl - (initial[x.idx] ?? 0) : x.lvl }));
      ...
      <div className="small" style={{ marginTop: 14 }}>
        Local dev: <span className="mono">cd web</span> → <span className="mono">npm install</span> → <span className="mono">npm run dev</span>
      </div>
    </div>
  );
}
